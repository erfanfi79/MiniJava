...
public class Parser {
    ...
    private final CodeGeneratorFacade cgf = new CodeGeneratorFacade();
...
//                        Log.print("");
                        try {
                            getCgf().semanticFunction(rule.semanticAction, lookAhead);
                        } catch (Exception e) {
...
//                    parsStack.pop();
        ...
        if (!ErrorHandler.hasError) getCgf().printMemory();
    }
    ...
    public CodeGeneratorFacade getCgf() {
        return cgf;
    }
}
